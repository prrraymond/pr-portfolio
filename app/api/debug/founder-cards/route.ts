...
export async function GET() {
  try {
    ...
    const report = {
      ...
      sortOrderCounts: founderItems.reduce(
        (acc, item) => {
          acc[item.sortOrder] = (acc[item.sortOrder] || 0) + 1
          return acc
        },
        {} as Record<string, number>,
      ),
    ...
    console.error("Error in founder-cards debug endpoint:", error)
    return NextResponse.json({ error: "Failed to analyze founder cards" }, { status: 500 })
  }
}
